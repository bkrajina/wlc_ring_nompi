...
!     Parallel tempering variables
      ...
      CHARACTER*3 LkPlusStr     !String variable for Lk replica one Lk step above current replica
      CHARACTER*3 LkMinusStr      !String variable for Lk replica one LK step below current replica
      CHARACTER*3 LkSwapStr     !String variable for Lk replica to test for exchange
...
!     Structure analysis
      ...
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: R4  !Second moment of end-to-end displacement 
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: R6  !Second moment of end-to-end displacement 
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: DR  !Second moment of end-to-end displacement 


      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: R2ALL !Second moment matrix
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: R4ALL !Second moment matrix
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: R6ALL !Second moment matrix
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:) :: DRALL !Second moment matrix
...
!     The current Lk. Also write them to character variables
      ...
      IF (ParTempOn.EQ.1) THEN
         ...
         ELSE
            ...
            LkMinus=Lk-Lkstep
         ENDIF
         write(LkMinusStr,fmt='((I3))') LkMinus
         write(LkPlusStr,fmt='((I3))') LkPlus
         TensMinus=nint(log10(1.*LkMinus)-0.4999)+1
         TensPlus=nint(log10(1.*LkPlus)-0.4999)+1
...
!     Save the conformation and the metrics
         ...
         IF (ParTempOn.EQ.1) THEN         
            ...
            DO WHILE (LkMinusInd.NE.IND.OR.LkPlusInd.NE.IND) 
               OPEN(unit=1,file='../LK_'//LkMinusStr((3-TensMinus+1):3)//'/data/MC_IND',IOSTAT=IOStatus,status='old')
               READ(unit=1,fmt=*,IOSTAT=IOStatus) LkMinusInd
               CLOSE(unit=1,IOSTAT=IOStatus)

               OPEN(unit=1,file='../LK_'//LkPlusStr((3-TensPlus+1):3)//'/data/MC_IND',IOSTAT=IOStatus,status='old')
            ...
            IF (UpOrDown.EQ.1) THEN
               ...
               OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/E_Current')
               ...
               OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/Wr_Current')
            ...
            IF (UpOrDown.EQ.-1) THEN
               ...
               DO WHILE (ReplicaExchangeIndex.NE.IND) 
                  OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/ExchangeIndex',IOSTAT=IOStatus,status='old')
               ...
               OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/Accept',IOSTAT=IOStatus,status='old')
            ...
            IF (Accept.EQ.1) THEN
               ...
               print *, 'accept'
               OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/r'//fileind((5-TENS+1):5),status='old')
               DO I=1,NT
                  READ(unit=1,fmt=*) R(I,:)
               ENDDO
               CLOSE(unit=1)
               
               OPEN(unit=1,file='../LK_'//LkSwapStr((3-TensSwap+1):3)//'/data/u'//fileind((5-TENS+1):5),status='old')
               DO I=1,NT
                  READ(unit=1,fmt=*) U(I,:)
               ENDDO
               CLOSE(unit=1)
...
10          CONTINUE
            ...
            DO WHILE (LkMinusComp.NE.IND.OR.LkPlusComp.NE.IND) 
               OPEN(unit=1,file='../LK_'//LkMinusStr((3-TensMinus+1):3)//'/data/MC_Completed',IOSTAT=IOStatus,status='old')
               READ(unit=1,fmt=*,IOSTAT=IOStatus) LkMinusComp
               CLOSE(unit=1,IOSTAT=IOStatus)

               OPEN(unit=1,file='../LK_'//LkPlusStr((3-TensPlus+1):3)//'/data/MC_Completed',IOSTAT=IOStatus,status='old')
